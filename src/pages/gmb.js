/**
 * GMB Reviews Page — Luxelens ReviewAI
...
export async function renderGmbReviews(container) {
  _showLoading(container);
  await api.fetchWebhookReviews('gmb');
...
function _showLoading(container) {
  container.innerHTML = `
    <div class="page-hero animate-fade-in">
      <div class="hero-watermark">GOOGLE</div>
      <div class="hero-content">
        <h1 class="hero-title">
          <span class="hero-dark">GMB&nbsp;</span><span class="hero-accent">REVIEWS</span>
        </h1>
        <p class="hero-subtitle">Monitor and respond to your Google My Business reviews in real-time.</p>
      </div>
    </div>
...
function _renderPage(container) {
  const reviews = api.getReviews().filter(r => r.platform === 'gmb');
...
function _attachListeners(container, reviews) {
  ...
  searchInput?.addEventListener('input', e => {
    ...
    debounce = setTimeout(() => {
      ...
      if (listEl) {
        listEl.innerHTML = _renderFiltered(api.getReviews().filter(r => r.platform === 'gmb'));
...
function _attachCardActions(container) {
  ...
  container.querySelectorAll('[data-action="generate-reply"]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const reply = review.starRating >= 4
        ? `Thank you so much for your kind words, ${review.authorName}! We're delighted to hear about your experience. Your feedback motivates us to keep delivering the best service possible!`
  ...
  container.querySelectorAll('[data-action="post-reply"]').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      const result = await api.postReply(review.reviewId, review.aiReply, 'gmb');
