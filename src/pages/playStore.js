/**
 * Play Store Reviews Page — Luxelens ReviewAI
...
export async function renderReviews(container) {
  ...
  await api.fetchWebhookReviews('playstore');
...
function _showLoading(container) {
  container.innerHTML = `
    <div class="page-hero animate-fade-in">
      <div class="hero-watermark">PLAY STORE</div>
      <div class="hero-content">
        <h1 class="hero-title">
          <span class="hero-dark">PLAY STORE&nbsp;</span><span class="hero-accent">REVIEWS</span>
        </h1>
        <p class="hero-subtitle">Manage and reply to your Google Play Store reviews with AI assistance.</p>
      </div>
    </div>
...
function _renderPage(container) {
  const reviews = api.getReviews().filter(r => r.platform === 'playstore' || !r.platform);
...
function _attachListeners(container, reviews) {
  ...
  searchInput?.addEventListener('input', e => {
    ...
    debounce = setTimeout(() => {
      ...
      if (listEl) {
        listEl.innerHTML = _renderFiltered(api.getReviews().filter(r => r.platform === 'playstore' || !r.platform));
...
function _attachCardActions(container) {
  ...
  container.querySelectorAll('[data-action="generate-reply"]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const reply = review.starRating >= 4
        ? `Thank you so much for your kind words, ${review.authorName}! We're thrilled you're enjoying Luxelens. Your support means a lot — we'll keep making your experience better!`
  ...
  container.querySelectorAll('[data-action="post-reply"]').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      const result = await api.postReply(review.reviewId, review.aiReply, 'playstore');
